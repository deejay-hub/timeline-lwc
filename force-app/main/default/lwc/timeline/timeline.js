...
export default class timeline extends NavigationMixin(LightningElement) {
    ...
    getTimelineRecords(result) {
        ...
        let timelineResult = [];
        let timelineTimes = [];
        const isMultiDay = this.isMultiDay;
        result.forEach(function (record, index) {
            ...
            const endDate = record.positionDateEndValue ? moment(record.positionDateEndValue, 'YYYY-MM-DD HH:mm:ss').toDate() : null;
            recordCopy.timeEnded = isMultiDay ? (moment(endDate).isAfter(recordCopy.time) ? endDate : null) : null;
